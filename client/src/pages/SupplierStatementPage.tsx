...
import { ArrowLeft, ShoppingCart, CreditCard, Download, AlertTriangle } from 'lucide-react';
...
const SupplierStatementPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchStatement = async () => {
      try {
        const data = await apiService.getSupplierStatement(parseInt(id!));
        setStatement(data);
      } catch (error) {
  ...
  if (!statement) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-500">Suivi non trouvé</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">
              Suivi fournisseur - {statement.supplier.name}
            </h1>
            <p className="text-gray-600 mt-1">
              NIF: {statement.supplier.nif} • Tél: {statement.supplier.phone}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="card">
          <div className="flex items-center">
            ...
            <div>
              <p className="text-sm font-medium text-gray-600">Total commandé</p>
              <p className="text-2xl font-bold text-gray-900">
                {formatCFA(statement.summary.totalOrdered)}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="card">
          <div className="flex items-center">
            ...
            <div>
              <p className="text-sm font-medium text-gray-600">Reliquat total</p>
              <p className="text-2xl font-bold text-gray-900">
                {statement.summary.totalRemainder.toLocaleString()} L
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Commandes d'achat */}
      <div className="card">
        <h2 className="text-xl font-semibold text-gray-900 mb-4">Commandes d'achat</h2>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="table-header">Qté commandée</th>
                <th className="table-header">Qté reçue</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {statement.purchaseOrders.map((po: any) => (
                <tr key={po.id} className="hover:bg-gray-50">
                  <td className="table-cell font-medium">{po.cmdNo}</td>
                  ...
                  <td className="table-cell">{po.qtyOrderedL.toLocaleString()} L</td>
                  <td className="table-cell">{po.totalReceived.toLocaleString()} L</td>
                  <td className="table-cell">
                    <span className={`font-medium ${po.remainder > 0 ? 'text-orange-600' : 'text-green-600'}`}>
                      {po.remainder.toLocaleString()} L
                    </span>
                  </td>
                  <td className="table-cell font-medium">
                    {formatCFA(po.totalValue)}
                  </td>
                  <td className="table-cell">
                    <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                      ...
                      {po.status === 'CLOSED' ? 'Fermée' :
                       po.status === 'RECEIVED' ? 'Reçue' :
                       po.status === 'ORDERED' ? 'Commandée' : 'Brouillon'}
                    </span>
                  </td>
                </tr>
              ))}
      ...
      {/* Paiements */}
      <div className="card">
        <h2 className="text-xl font-semibold text-gray-900 mb-4">Paiements effectués</h2>
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
